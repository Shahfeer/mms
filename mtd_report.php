...
site_log_generate("MTD Report Page : User : " . $_SESSION['yjtmms_user_name'] . " access the page on " . date("Y-m-d H:i:s"));

$thismonth_startdate = date("Y-m-01");
// $thismonth_yesterday = date('Y-m-d', strtotime("-1 days"));
// $thismonth_yesterday =  date('Y-m-t', strtotime("this month"));
if (date('Y-m-d', strtotime("-1 days")) == date('Y-m-t', strtotime("previous month"))) {
	$thismonth_yesterday = date('Y-m-t', strtotime("this month"));
} else {
	$thismonth_yesterday = date('Y-m-d', strtotime("-1 days"));
}
...
if (strtotime($thismonth_startdate) > strtotime($thismonth_yesterday)) {
...
// $api_url='http://localhost:10017/report/mtd_report';
// $bearer_token = 'Authorization: ' . $_SESSION['yjtsms_user_bearer_token'];


//send api response
$curl = curl_init();
$api_url='http://192.168.29.58:8084/report/mtd_report';
$bearer_token = 'Authorization: ' . $_SESSION['yjtsms_user_bearer_token'];
...
$response = curl_exec($curl); //storing response
$http_status = curl_getinfo($curl, CURLINFO_HTTP_CODE);

curl_close($curl);


if ($response === false || $http_status==403) {  //if response is false
	echo '<script>window.location = "logout"</script>';
}
$data = json_decode($response, true);  //storing response in $data variable and seperate response




if (isset($data['mtd_result']) && is_array($data['mtd_result'])) {
	...
	foreach ($data['mtd_result'] as $report) {
		...
		$others_cnt_array = [];
		$registration_cnt_array = [];
	}; // Handle error if API response format is unexpected
...
<head>
	...
	<title>MTD Report : <?= $site_title ?></title>
			...
			<div class="layout-page">
				...
				<div class="content-wrapper">
					...
					<div class="container-xxl flex-grow-1 container-p-y">
						<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Home /</span> MTD Report</h4>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="card mb-4">
									...
									<div class="card-body">
										<form action="#" method="post" class="j-pro" id="j-pro" novalidate=""
											...
											<div class="table-responsive dt-responsive">
												...
												<div class="container">
													<div style="text-align: center; font-size: 1.4rem; font-weight: bold;">MTD Report Duration :
														<?= $thismonth_startdate ?> - <?= $thismonth_yesterday ?></div>
													<div style="height: 400px !important; width: 100% !important;">
														<script> //storing array in variables
									...
									<div class="mb-5">
										<form action="#" method="post" class="j-pro" id="j-pro1" novalidate=""
											style="border:0px !important;">
											<!-- Responsive Table -->
											<div class="table-responsive dt-responsive">
												...
												<div class="container">
													<table id="employee-grid" class="table table-striped table-bordered"
														...
														if (count($visitor_cnt_array) > 0) {
															for ($i = 0; $i < count($entry_date_array); $i++) {
																$ii++; ?>
																<tr>
																	<td><?= $i + 1 ?></td>
