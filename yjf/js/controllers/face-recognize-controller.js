function FaceRecognizeController($scope, $state, $interval, $http, config, $timeout) {
...
if ($scope.faces_collection.length != 0) {
             ...
             attn_name=$scope.faces_collection[0].metadata.name;
             attn_number=$scope.faces_collection[0].metadata.number;
             attn_face_id=$scope.faces_collection[0].metadata.faceId;
             attn_image=$scope.faces_collection[0].metadata.image;
//             console.log(attn_name);
//            getNumber.push(faces_collection);
        $http({
              method: 'POST',
              url: 'php/insert.php',
              headers: {'Content-Type': 'application/json'},
                data: JSON.stringify({
                    attn_name:attn_name,
                    attn_number:attn_number,
                    attn_face_id:attn_face_id,
                    attn_image:attn_image
                })
          }).then(function (response) {
              console.log(response.data.status)
              if(response.data.status==0){
    //  toastr.success(attn_name+' attendance updated.');
...
angular.module('face-recognize-controller', []).controller('FaceRecognizeController', FaceRecognizeController);
