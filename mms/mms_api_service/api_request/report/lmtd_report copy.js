...
const today = new Date();
        ...
        // Calculate the corresponding date in the previous month (i.e., yesterday's date but one month earlier)
        const previousMonthYesterday = new Date(today);
        previousMonthYesterday.setMonth(today.getMonth() - 1);
        previousMonthYesterday.setDate(today.getDate() - 1);
        
        // Calculate difference in days between previous month start date and the corresponding date in the previous month
        const differenceInDaysPreviousMonth = Math.floor((previousMonthYesterday - previousMonthStartDate) / (1000 * 60 * 60 * 24));
        // console.log(previousMonthStartDate);
        console.log(differenceInDaysPreviousMonth);
        
        // Print the results
        console.log(`Previous month start date (${formatDate(previousMonthStartDate)}) to corresponding date last month (${formatDate(previousMonthYesterday)}): ${differenceInDaysPreviousMonth} days`);
