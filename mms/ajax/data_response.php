...
function getEMP($conn)
{
        ...
        $report = $responseData['data']['report'];
         ...
         if (!empty($requestData['search']['value'])) {
          ...
          $report = array_filter($report, function($row) use ($searchValue) {
              return strpos(strtolower((string)$row['visitor_id']), $searchValue) !== false
                  || strpos(strtolower((string)$row['aws_faceid']), $searchValue) !== false
                  || strpos(strtolower((string)$row['customer_name']), $searchValue) !== false
                  || strpos(strtolower((string)$row['customer_mobile']), $searchValue) !== false
                  || strpos(strtolower((string)$row['customer_gender']), $searchValue) !== false
                  || strpos(strtolower((string)$row['min_age']), $searchValue) !== false
                  || strpos(strtolower((string)$row['max_age']), $searchValue) !== false
                  || strpos(strtolower((string)$row['age_category']), $searchValue) !== false
                  || strpos(strtolower((string)$row['interest_list_master']), $searchValue) !== false;
          });
      }
    ...
    } else if($responseData['response_status'] == 403 || $response == false){
        $json_data = array(
  ...
  if ($responseData['response_code'] == 1 && !empty($responseData['data']['report'])) {
    ...
    if (!empty($requestData['search']['value'])) {
      ...
      $data = array_filter($data, function($row) use ($searchValue) {
          return stripos(strtolower((string)$row['visitor_id']), $searchValue) !== false
              || stripos(strtolower((string)$row['aws_faceid']), $searchValue) !== false
              || stripos(strtolower((string)$row['customer_name']), $searchValue) !== false
              || stripos(strtolower((string)$row['customer_mobile']), $searchValue) !== false
              || stripos(strtolower((string)$row['visitor_gender']), $searchValue) !== false
              || stripos(strtolower((string)$row['min_age']), $searchValue) !== false
              || stripos(strtolower((string)$row['max_age']), $searchValue) !== false
              || stripos(strtolower((string)$row['age_category']), $searchValue) !== false;
      });
  }
...
function getRTHRS($conn) {
    ...
    if ($response_data['response_status'] == 403 || $response == false) {
        $json_data = array(
            "draw" => intval($requestData['draw']),
            "recordsFiltered" => 403,
            "data" => []
    ...
    if (!empty($requestData['search']['value'])) {
        $searchValue = strtolower($requestData['search']['value']);
        $data = array_filter($data, function($row) use ($searchValue) {
            return stripos(strtolower((string)$row['visitor_list_entdate']), $searchValue) !== false
                || stripos(strtolower((string)$row['hour_time']), $searchValue) !== false
                || stripos(strtolower((string)$row['visitor_count']), $searchValue) !== false
                || stripos(strtolower((string)$row['male_cnt']), $searchValue) !== false
                || stripos(strtolower((string)$row['female_cnt']), $searchValue) !== false
                || stripos(strtolower((string)$row['others_cnt']), $searchValue) !== false
                || stripos(strtolower((string)$row['cnt_registration']), $searchValue) !== false;
        });
    }
    ...
    $json_data = array(
        ...
        'data' => array_values($data) // Data to be displayed
    );
